...
import ru.elron.examplerecycleradapter.view.DragAndDropCallback
...
class LongClickViewModel : AndroidViewModel, OnItemClickViewHolderCallback {
    val adapter = RecyclerAdapter<LongClickObservable>()

    constructor(application: Application, state: SavedStateHandle) : super(application) {
        LongClickViewHolder.addViewHolder(adapter.holderBuilderArray, this)

        val names = application.resources.getStringArray(R.array.names)
        for (i in 0..names.lastIndex)
            adapter.observableList.add(LongClickObservable(names[i]))
    }
    ...
    fun setupDragAndDrop(recyclerView: RecyclerView) {
        LongClickViewHolder.ITEM_TOUCH_HELPER = ItemTouchHelper(DragAndDropCallback())
        LongClickViewHolder.ITEM_TOUCH_HELPER!!.attachToRecyclerView(recyclerView)
    }

    override fun onCleared() {
        LongClickViewHolder.ITEM_TOUCH_HELPER = null
        super.onCleared()
    }
}

class LongClickViewModelFactory(
    ...
    override fun <T : ViewModel?> create(
        ...
        return LongClickViewModel(application, handle) as T
    }
}
